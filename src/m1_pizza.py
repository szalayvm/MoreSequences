...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # Test 3:
    title = 'DRAW_POINTS_ON_CIRCLE, test 3:  80 purple dots on a black circle.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  4 slices, thin (thickness=3) yellow lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 7, 'yellow', 3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    #Test 3:
    title = 'POLYGON test 3: 7 segments with green lines'
    window = rg.RoseWindow(550,400,title)
    circle = rg.Circle(rg.Point(100,100),80)
    circle.outline_thickness = 3
    circle.fill_color = 'lime green'
    polygon(window,circle, 7,'green',5)
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    # Test 4
    title = 'FANCY POLYGON test 4:  20 red lines on yellow circle, hops = 5.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 20, 7, 'red', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(list)):
        if k + hops_to_next_point > len(list)-1:
            add = (k+hops_to_next_point) % len(list)
            n = 0
        else:
            add = hops_to_next_point
            n = k
        start_point = list[k]
        end_point = list[n + add]
        line = rg.Line(start_point, end_point)
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
